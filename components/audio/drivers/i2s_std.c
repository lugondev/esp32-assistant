...
static void i2s_set_volume(int pct) {
    if (pct < 0) pct = 0;
    if (pct > 100) pct = 100;
    s_volume = pct;
}
static int i2s_get_volume(void) { return s_volume; }
static int i2s_adjust_volume(int delta) {
...
static esp_err_t i2s_init(const void *cfg_v) {
    ...
    i2s_std_config_t rx_std = {
        ...
        .gpio_cfg = {
            .mclk = I2S_GPIO_UNUSED, .bclk = c->mic_sck,
            .ws = c->mic_ws, .dout = I2S_GPIO_UNUSED,
            .din = c->mic_sd,
        },
    };
    ...
    i2s_std_config_t tx_std = {
        ...
        .gpio_cfg = {
            .mclk = I2S_GPIO_UNUSED, .bclk = c->spk_bclk,
            .ws = c->spk_lrc, .dout = c->spk_din,
            .din = I2S_GPIO_UNUSED,
        },
    };
...
static int i2s_mic_read(int16_t *pcm, int samples) {
...
static int i2s_spk_write(const int16_t *pcm, int samples) {
    int vol = s_volume;
...
static void i2s_spk_reset(void) {
    xSemaphoreTake(s_tx_mutex, portMAX_DELAY);
